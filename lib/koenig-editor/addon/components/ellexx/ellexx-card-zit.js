...
import utils from './utils/ellexx-card-utils';
...
export default EllexxComponent.extend({
    ...
    setDefaultValueIfNeeded() {
        ...
        ellexxCardUtils.updatePayloadAttribute(this.payload, 'zit_usage', 'quick-content', this.saveCard);
    },
    ...
    updateUsage: action(function (event) {
        ...
        if (this.payload.zit_usage === 'quick-content') {
            ...
            utils.updatePayloadAttribute(this.payload, 'zit_quote', this.payload.zit_quote ? this.payload.zit_quote.substring(0, MAX_QUICK_CONTENT) : '', this.saveCard);
            this.set('zit_quote_length', this.payload.zit_quote ? this.payload.zit_quote.length : 0);
        } else {
