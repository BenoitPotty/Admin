...
const IMAGE_MIME_TYPES = 'application/pdf';
...
export default EllexxComponent.extend({
    ...
    init() {
        this._setDefaultValueIfNeeded();
        this._super(...arguments);
        this.initField('pdf_visibility');
        ...
        /* BPY */
        this._adaptPreviewSrcToPdf(this.payload.src);
    ...
    actions: {
        updateSrc(images) {
            ...
            this.editor.run(() => {
                ...
                /* BPY : Tweak by updating preview source with the Rokka stack, but we want a JPG not a PDF */
                this._adaptPreviewSrcToPdf(image.url);
    ...
    _adaptPreviewSrcToPdf(url) {
        if (url) {
            this.set('previewSrc', url.replace('.pdf', '.jpg'));
        } else {
            this.set('previewSrc', url);
        }
    },

    _setDefaultValueIfNeeded() {
        if (!this.payload) {
            this.payload = {
                pdf_visibility: 'free'
            };
        ...
        ellexxCardUtils.updatePayloadAttribute(this.payload, 'pdf_visibility', 'free', this.saveCard);
    }
});
